...
public class TaskDelivery {
	...
	public static void assignTask(Long userId, Long taskId, double latitude,
		...
		System.out.println("TaskDelivery-->receive a task " + taskId + "/"
				+ latitude + "/" + longtitude + "/" + task.getSensorType()
				+ "/" + task.getSearchRange());
		List<User> suitableUsers = um.suitableUsers(latitude, longtitude,
				task.getSensorType(), task.getSearchRange());
		System.out.println("TaskDelivery-->find suitableUsers how many:"
				+ suitableUsers.size());
		String device;
		String message;
		if (suitableUsers.size() != 0) {
			...
			for (int i = 0; i < suitableUsers.size(); i++) {
				if (!suitableUsers.get(i).getUserId().equals(userId)) {
					User user = um.getCertainUser(suitableUsers.get(i)
							.getUserId());
					TaskAssignment ta = new TaskAssignment();
					Long assignId;
					if (i != 0) {
						// create a new task to user.
						Long newTaskId = tm.createMainTask(task.getQueryId(),
								task.getCategoryId(), latitude, longtitude,
								streetName);
					...
					message = task.getDescription() + "_"
							+ task.getExpirePeriod() + "_" + task.getTaskId()
							+ "_" + assignId;
					System.out.println("MESSAGE=" + message);
					// send message out, return infor if sending successful.
					System.out.println(GCMUtilities
